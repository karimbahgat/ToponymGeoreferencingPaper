...
def shapediff_prepped(polys, g2):       
    ...
    dsfdfdsfdsf
    for poly in polys:
        ext = poly.boundary # exterior only
        if 'Multi' in g2.geom_type:
            fds
        else:
            dists = []
...
def find_match(test, pool):
    ...
    g2s = []
    for f in pool:
        if 'Multi' in f.geometry['type']:
            norm2 = {'type':'MultiPolygon',
                     'coordinates':[{'type':'Polygon', 'coordinates':normalize(poly)['coordinates']
...
def find_match_prepped(test, pool):
    ...
    t=time()
    if 'Multi' in test['type']:
        norm1 = {'type':'MultiPolygon',
                 'coordinates':[{'type':'Polygon', 'coordinates':normalize(poly)['coordinates']
                                 for poly in test['coordinates']}]}
    else:
        norm1 = normalize(test)
    simple = shapely.geometry.asShape(norm1).simplify(0.1)
    if 'Multi' in simple.geom_type:
        polys = [g for g in simple.geoms]
    else:
        polys = [simple]
    # (only if offset mode)
