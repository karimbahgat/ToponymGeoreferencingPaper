...
if __name__ == '__main__':
    ...
    diff = color_changes(quantize(im))
    PIL.Image.fromarray(diff).show()
    edges = (diff > 10).astype(np.uint8) * 255
    ...
    chararea = 5
    eps = 3 # allowable pixel error
    chars = [cnt for cnt in contours if abs(getheight(cnt)-chararea) < eps ]

    im_arr_draw = np.zeros(edges.shape)
    cv2.drawContours(im_arr_draw, chars, -1, 255, -1)
