...
##    homogim.show()
    ...
    # OR pick only homogenous pixels lower than regional avg
##    from PIL import ImageFilter
##    radius = 1
##    mean = PIL.Image.fromarray(zvals*255).convert('L').filter(ImageFilter.BoxBlur(radius))
##    mean.show()
##    mean = np.array(mean) / 255.0
##    homogmask = zvals < mean #homogthresh # opposite bc want to reset where not homogenous
...
##    im.show()
    ...
    # get prototype colors
    protos = find_prototype_colors(im)
