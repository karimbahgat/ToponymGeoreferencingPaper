...
def color_difference(im, color):
    ...
    #diff_im[dissim] = thresh
    #diff_im = (diff_im - diff_im.min()) / float(diff_im.max()) * 255.0
...
if __name__ == '__main__':
    ...
    def sniff_text_colors(im, samples=15):
        ...
        sw,sh = 400,400
        texts = []
        for i,q in enumerate(sample_quads(im, (sw,sh))):
            print '---'
            print 'sample',i,q
            x1,y1,x2,y2 = q.bbox()
...
##        twtimes = round(w/float(tw))
##        thtimes = round(h/float(th))
##        tw = int(w/twtimes) + 1
##        th = int(h/thtimes) + 1
##
##        # max half img size
...
##                yield q
    ...
    def sample_texts(im, textcolors, samplesize=(400,400)):
        from random import uniform
        import re
        w,h = im.size
        sw,sh = samplesize
...
##                lmask[diffmask] = lmask.max() # cap max luminance to parts that are too different
                ...
                # detect text
                print 'running ocr'
                data = automap.main.detect_data(lmaskim)
                ...
                for text in data:
                    ...
                    if float(text['conf']) > 50 and len(text['text']) >= 2:
                        ...
                        text['left'] = x1 + float(text['left'])
                        text['top'] = y1 + float(text['top'])
                        texts.append(text)
